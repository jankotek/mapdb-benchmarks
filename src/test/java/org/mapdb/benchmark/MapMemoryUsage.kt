...
class MapMemoryUsage{
    ...
    companion object{
        @JvmStatic fun main(args : Array<String>){
            val map = when(args[0]) {
                "HTreeMap" -> DBMaker.memoryDB().make().hashMap("map", Serializer.LONG, Serializer.UUID).valueInline().make()
                "BTreeMap" -> DBMaker.memoryDB().make().treeMap("map", Serializer.LONG_DELTA, Serializer.UUID).make()
                "HTreeMap_heap" -> DBMaker.heapDB().make().hashMap("map", Serializer.LONG, Serializer.UUID).valueInline().make()
                "BTreeMap_heap" -> DBMaker.heapDB().make().treeMap("map", Serializer.LONG_DELTA, Serializer.UUID).make()
